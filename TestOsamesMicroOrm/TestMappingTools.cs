...
namespace TestOsamesMicroOrm
{
    ...
    public class TestMappingTools : OsamesMicroOrmTest
    {
        ...
        {
            ConfigurationLoader.FillMappingDictionary(new XPathDocument(_mappingFileFullPath).CreateNavigator(), "orm", "http://www.osames.org/osamesorm");
            Assert.AreEqual("CustomerId", MappingTools.GetColumnName(new Customer().GetType().GetProperty("IdCustomer")));
            Assert.IsNull(MappingTools.GetColumnName(new Customer().GetType().GetProperty("ThisPropertyDoesntExist")));
            Assert.IsNull(MappingTools.GetColumnName(new TestUnmappedEntity().GetType().GetProperty("Id")));
        }
