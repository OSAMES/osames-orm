    ...
    public sealed class DbManager
    {
        ...
        public int ExecuteNonQuery(DbConnectionWrapper connection_, CommandType cmdType_, string cmdText_, object[,] cmdParams_, out long lastInsertedRowId_)
        {
            ...
            if (connection_.IsBackup)
            {
                lock (BackupConnectionUsageLockObject)
                {
                    ...
                    using (DbCommandWrapper command = new DbCommandWrapper(connection_, cmdText_, cmdParams_, cmdType_))
                    {
                        iNbAffectedRows = command.ExecuteNonQuery();
                    }

                    using (DbCommandWrapper command = new DbCommandWrapper(connection_, SelectLastInsertIdCommandText, (object[,])null))
                    {
        ...
        public int ExecuteNonQuery(DbTransactionWrapper transaction_, CommandType cmdType_, string cmdText_, object[,] cmdParams_, out long lastInsertedRowId_)
        {
            ...
            if (transaction_.Connection.IsBackup)
            {
                ...
                {
                    // perform code with locking
                    using (DbCommandWrapper command = new DbCommandWrapper(transaction_.Connection, cmdText_, cmdParams_, cmdType_))
                    {
                        iNbAffectedRows = command.ExecuteNonQuery();
                    }

                    using (DbCommandWrapper command = new DbCommandWrapper(transaction_.Connection, SelectLastInsertIdCommandText, (object[,])null))
                    {
                        object oValue = command.ExecuteScalar();
                        if (!Int64.TryParse(oValue.ToString(), out lastInsertedRowId_))
                            throw new Exception("Returned last insert ID value '" + oValue + "' could not be parsed to Long number");
        ...
        public int ExecuteNonQuery(DbConnectionWrapper connection_, CommandType cmdType_, string cmdText_, IEnumerable<DbCommandWrapper.Parameter> cmdParams_, out long lastInsertedRowId_)
        {
            ...
            if (connection_.IsBackup)
            {
                lock (BackupConnectionUsageLockObject)
                {
                    ...
                    using (DbCommandWrapper command = new DbCommandWrapper(connection_, SelectLastInsertIdCommandText, (IEnumerable<DbCommandWrapper.Parameter>)null))
                    {
                        object oValue = command.ExecuteScalar();
                        if (!Int64.TryParse(oValue.ToString(), out lastInsertedRowId_))
                            throw new Exception("Returned last insert ID value '" + oValue + "' could not be parsed to Long number");
        ...
        public int ExecuteNonQuery(DbTransactionWrapper transaction_, CommandType cmdType_, string cmdText_, IEnumerable<DbCommandWrapper.Parameter> cmdParams_, out long lastInsertedRowId_)
        {
            ...
            if (transaction_.Connection.IsBackup)
            {
                lock (BackupConnectionUsageLockObject)
                {
                    ...
                    using (DbCommandWrapper command = new DbCommandWrapper(transaction_.Connection, SelectLastInsertIdCommandText, (IEnumerable<DbCommandWrapper.Parameter>)null))
                    {
                        object oValue = command.ExecuteScalar();
                        if (!Int64.TryParse(oValue.ToString(), out lastInsertedRowId_))
                            throw new Exception("Returned last insert ID value '" + oValue + "' could not be parsed to Long number");
        ...
        public int ExecuteNonQuery(DbConnectionWrapper connection_, CommandType cmdType_, string cmdText_, IEnumerable<KeyValuePair<string, object>> cmdParams_, out long lastInsertedRowId_)
        {
            ...
            if (connection_.IsBackup)
            {
                lock (BackupConnectionUsageLockObject)
                {
                    ...
                    using (DbCommandWrapper command = new DbCommandWrapper(connection_, SelectLastInsertIdCommandText, (IEnumerable<KeyValuePair<string, object>>)null))
                    {
                        object oValue = command.ExecuteScalar();
                        if (!Int64.TryParse(oValue.ToString(), out lastInsertedRowId_))
                            throw new Exception("Returned last insert ID value '" + oValue + "' could not be parsed to Long number");
        ...
        public int ExecuteNonQuery(DbTransactionWrapper transaction_, CommandType cmdType_, string cmdText_, IEnumerable<KeyValuePair<string, object>> cmdParams_, out long lastInsertedRowId_)
        {
            ...
            if (transaction_.Connection.IsBackup)
            {
                lock (BackupConnectionUsageLockObject)
                {
                    ...
                    using (DbCommandWrapper command = new DbCommandWrapper(transaction_.Connection, SelectLastInsertIdCommandText, (IEnumerable<KeyValuePair<string, object>>)null))
                    {
                        object oValue = command.ExecuteScalar();
                        if (!Int64.TryParse(oValue.ToString(), out lastInsertedRowId_))
                            throw new Exception("Returned last insert ID value '" + oValue + "' could not be parsed to Long number");
